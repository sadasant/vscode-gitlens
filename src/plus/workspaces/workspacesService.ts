...
import type { WorkspaceRepositoryDescriptor, WorkspacesResponse } from './models';
import { GKCloudWorkspace, WorkspaceType } from './models';
		...
		if (workspaces?.length) {
			for (const workspace of workspaces) {
				...
				cloudWorkspaces.push(
					new GKCloudWorkspace(WorkspaceType.Cloud, workspace.id, workspace.name, repositories),
				);
			}
		}
